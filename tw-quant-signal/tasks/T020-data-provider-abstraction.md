...
- [ ] 建立 `src/tw_quant_signal/provider/` 目錄
- [ ] 定義 `DataProvider` 抽象類別（含所有 fetch_* 方法的「簽名」）:
...
- [ ] 將 `twse_client.py` 的 `fetch_*` 函式群封裝為 `TwseDirectProvider(DataProvider)`
- [ ] `twse_client.py` 保留為 `TwseDirectProvider` 的內部實作，不暴露匯出層級函式
- [ ] `WATCH_STOCKS` 常數從 `twse_client.py` 移至 `config.py` 或 `provider/__init__.py`

### S3: yfinance 封裝 — `YfinanceProvider`
- [ ] 將 `twse_client.py` 中的 yfinance 相關函式（`fetch_yf_quarterly_financials_batch`、`fetch_yf_financials`、`_fetch_dividends_yf`）封裝為 `YfinanceProvider(DataProvider)`
- [ ] `YfinanceProvider` 作為 `TwseDirectProvider` 的「補充提供者」，只在某些財務資料上使用

### S4: Provider 註冊與工廠
- [ ] `provider/__init__.py` 實作 `create_data_provider(mode: str = "direct") -> DataProvider`
- [ ] 支援模式：
  - `"direct"` — 現有 HTTP 直連（TwseDirectProvider + YfinanceProvider）
  - `"mcp"` — 呼叫 tw-quant-mcp（預留給 T021/T022）
- [ ] 環境變數 `TW_QUANT_DATA_PROVIDER=direct|mcp` 切換

### S5: 上游模組遷移至 DataProvider
- [ ] `ingestion.py:IngestionEngine` 改接收 `DataProvider` 實例取代直接 import `twse_client` 的函式
- [ ] `features.py` 改從 DataProvider 取得估值資料（替代原 inline `fetch_valuations`）
- [ ] `backtest.py` 改從 DataProvider 取得歷史資料
- [ ] `backfill.py` 改從 DataProvider 取得回補資料

### S6: 向後相容
- [ ] 設定 `TW_QUANT_DATA_PROVIDER=direct` 時行為與現有程式碼完全一致（diff 測試通過）
- [ ] 所有既有單元測試（T017 產出）在 `direct` 模式下仍通過
