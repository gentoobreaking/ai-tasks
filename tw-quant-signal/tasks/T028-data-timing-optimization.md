...
## 驗收標準
- [ ] 管線執行前檢查目標日期是否為當日，若是則跳過當日盤後資料 (融資融券、法人買賣超、月營收等)
- [ ] 改為抓取「前一交易日」的盤後資料
- [ ] 加入資料可用性檢查：嘗試抓取前先檢查資料是否已發布
- [ ] 降低不必要的錯誤日誌噪音 (如 "代碼 2330 於 2026-08-19 無融資融券資料")
- [ ] 更新 `ingestion.py` 的 `_ingest_margin_trading`、`_ingest_institutional_flows` 等函數
