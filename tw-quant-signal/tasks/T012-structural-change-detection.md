...
## 已交付檔案
```
src/tw_quant_signal/structural_change.py   ← 核心模組 (4 類偵測 + 報告 + 儲存)
src/tw_quant_signal/db.py                  ← + structural_drift 表 + get_structural_drift()
src/tw_quant_signal/pipeline.py            ← + 結構變化偵測步驟 (含 critical/warning 通知)
src/tw_quant_signal/api/app.py             ← + GET /api/structural-drift, /api/drift-report
```
