...
## 驗收標準
- [x] 在 `config.json` 新增 `mcp_timeout_sec` 參數（預設 60 秒）
- [x] `McpClient` 讀取該設定並應用於 HTTP client timeout
- [x] `McpProvider` 建立 client 時傳遞 timeout 參數
- [x] 可透過環境變數 `MCP_CALL_TIMEOUT` 覆蓋
- [x] 更新 `config.json` 範例與說明文件
