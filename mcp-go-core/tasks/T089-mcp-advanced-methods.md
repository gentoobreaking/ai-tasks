...
- [x] Server emits `notifications/resources/update` (implemented) and `notifications/resources/deleted` on resource removal (implemented via DeleteResource → NotifyResourceDeleted). See T104.
...
## 執行紀錄（2026-09-05 稽核）
- 已達成 4 項並打勾 (1-4, 6, 7)。
- **已實現**: `notifications/resources/deleted` — IMPLEMENTED via T104 `DeleteResource()` + `NotifyResourceDeleted()` + `ResourceDeletedNotification` protocol type。
