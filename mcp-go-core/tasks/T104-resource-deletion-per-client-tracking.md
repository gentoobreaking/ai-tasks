...
## 驗收標準
- [ ] `Router.DeleteResource(uri)` / `RemoveResource(uri)` method that removes resource and emits `notifications/resources/deleted` to subscribed clients
- [ ] `Router.UnsubscribeAll(uri)` removes all subscriptions for a URI (from T102)
- [ ] Per-client subscription tracking: `Subscribe(uri, clientID)` / `Unsubscribe(uri, clientID)` replacing global `map[string]bool`
- [ ] Backwards-compatible: `IsSubscribed(uri)` still works (returns true if any client subscribed)
- [ ] Protocol type: `ResourceDeletedNotification` with URI
- [ ] 3 tests: delete-notifies-subscriber, per-client-unsubscribe, is-subscribed-backwards-compat
