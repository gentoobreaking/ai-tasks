...
## 驗收標準
- [ ] 分診報告產出時註冊 pending approval：mutating 動作建立 ApprovalGate 請求，
      callback_id ↔ request_id ↔ incident_id 對映射表入 store（重啟可回復）
- [ ] ActionCallback handler 路由：approve/reject/snooze → ApprovalGate 對應方法
      （經 InteractionRouter RBAC 檢查）；拒絕原因即時入 RAG（§B.5 已有元件，接線驗證）
- [ ] 批准後呼叫 ExecutorRunner.execute(report)：執行結果逐步寫時間線並推播
- [ ] 逾時升級鏈運作：ApprovalGate timeout 由 daemon 排程驅動（非僅測試手動呼叫）
- [ ] e2e：警報→分診報告→按✅批准→executor dry-run+執行→時間線完整軌跡
