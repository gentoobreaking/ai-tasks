...
## 驗收標準
- [ ] config.py 新增：OPENAI_API_URL、TELEGRAM_SEND_URL 等常數（env 可覆寫，附預設）
- [ ] embedding.py / auto_develop.py / worker.py / telegram_bot.py / doctor.py / orchestrator.py
  全改走 config 常數；`rg "api.openai.com|api.telegram.org|redis://localhost"`（非 config/測試）無殘留
- [ ] 行為不變：env 值與現況預設一致時輸出相同；telegram_bot/worker/doctor 測試維持通過
