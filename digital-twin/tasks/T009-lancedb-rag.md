...
3. 保留 Git-backed 增量索引流程：pre-commit hook 觸發
4. 支援 `rag_query --version <tag>` 歷史回溯
4. 新增 BM25 + Vector 混合檢索（Gemini 第 2 輪建議）

## 驗收標準
- `git commit` 觸發增量索引
- `rag_query --version v20241201` 可查詢歷史版本
- 混合檢索（BM25 + Vector）精準定位程式碼片段
