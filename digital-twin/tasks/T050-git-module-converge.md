...
## 驗收標準
- [ ] common/git.py 提供：`git_diff_numstat(code_dir)`（等價 :791/:847 行為，含 staging）、
  `git_commit(code_dir, msg) -> hash|None`、`git_changed_files(code_dir) -> list[str]`、
  `git_revert_all(code_dir) -> bool`
- [ ] auto_develop 五處全改走 common/git.py；`rg "subprocess.run\(\[\"git\"" auto_develop.py` 無殘留
- [ ] PR 摘要閘門輸出與先前一致（test_auto_develop_deps 的 count_diff_lines/build_pr_summary 測試維持通過）
- [ ] pytest 全量維持 151 passed + 1 skipped；ruff 全過；不引入新依賴
