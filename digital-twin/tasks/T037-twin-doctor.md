...
## 驗收標準
- [ ] 新增（可在 twin 內或獨立 `doctor.py`）`./twin doctor`：依序檢查並色碼輸出
...
- [ ] doctor 結束給「總體狀態」一行（OK / 有警告 WARN / 有錯誤 FAIL，exit 0/1/2）
- [ ] 相依既有能力：不新增直接依賴，全部透過現有指令/env 讀取
- [ ] 文件：README 快速開始 §6 之後補 `./twin doctor` 使用說明
- [ ] pytest 全量仍 124 passed；ruff 全過（新增 doctor 測試 ≥2：缺 key 離線情境、全綠情境）
