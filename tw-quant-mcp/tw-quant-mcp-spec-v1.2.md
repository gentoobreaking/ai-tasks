...
### 3.11 當沖與盤中量能數據模組 (Intraday & Day-Trading Module)

為了支援動能當沖與盤中即時風控，本模組特別擴充對當沖資格檢查、即時 VWAP 與急拉爆量偵測的 MCP Tools 支援：

#### 1. 當沖資格與風控掃描工具 (`scan_daytrade_eligibility`)

* **功能描述：** 檢查目標個股今日是否開放當日沖銷（含先買後賣、先賣後買資格）、是否為處置股/注意股、是否有當沖限制。
* **Input Schema：**
```json
{
  "symbol": "2330"
...
```json
{
  "symbol": "2330",
  "can_daytrade": true,
  "can_short_first": true,       // 是否可先賣後買
  "is_disposition": false,      // 是否處置中
  "is_attention": false,        // 是否注意股
  "margin_short_available": true,
  "risk_level": "LOW",
  "restricted_reason": ""
}
...
#### 2. 盤中成交均價線與支撐壓力位 (`get_intraday_vwap`)

* **功能描述：** 計算盤中即時 **VWAP (Volume Weighted Average Price)**，並算出當日開盤高低點與 Fibonacci 支撐壓力位，作為當沖進出場依據。
* **Input Schema：**
```json
{
  "symbol": "2308"
...
```json
{
  ...
  "current_price": 345.5,
  "vwap": 342.1,               // 今日成交均價線
  "price_to_vwap_ratio": 0.0099, // 偏離度 (+0.99%)
  "day_high": 348.0,
  "day_low": 339.0,
  "support_levels": [342.1, 339.0],
  "resistance_levels": [348.0, 351.5]
}

```

#### 3. 盤中動能與急拉爆量偵測 (`detect_volume_surge`)

* **功能描述：** 監控個股近期（如近 1/3/5 分鐘）是否出現異常成交量暴增或價格急拉/急殺訊號。
* **Input Schema：**
```json
{
  "symbol": "2308",
...
```json
{
  "symbol": "2308",
  "is_surge": true,
  "surge_type": "BULLISH_BREAKOUT", // BULLISH_BREAKOUT / BEARISH_BREAKDOWN / NONE
  "volume_ratio": 3.45,            // 成交量為前 20 分鐘均量的 3.45 倍
  "price_change_pct": 1.48,        // 近 3 分鐘漲幅
  "alert_message": "近 3 分鐘爆量突破 VWAP 與盤中高點"
}
