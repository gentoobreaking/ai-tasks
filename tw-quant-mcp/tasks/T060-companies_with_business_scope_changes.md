---
github_issue: N/A
title: 新增工具 get_companies_with_business_scope_changes（行情歷史與指數）
type: feature
priority: medium
status: done
depends_on:
...
# T060 - 新增工具 `get_companies_with_business_scope_changes`（行情歷史與指數）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_companies_with_business_scope_changes`，
提供「查詢上市公司經營權及營業範圍異(變)動專區-營業範圍重大變更公司。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
## 驗收標準
- [x] `tools/list` 可見 `get_companies_with_business_scope_changes`，inputSchema 與本任務附帶者語意一致
...
## 執行紀錄（2026-08-25 稽核）
- 驗收標準逐條對照程式碼與測試後勾選。
- 證據：registry 註冊＋TestAllToolsEnvelopeConsistent 全工具 probe、snapshots/raw/get_companies_with_business_scope_changes.json、TestAllToolsCacheConsistency 全工具覆蓋、go vet/go test 全綠。
