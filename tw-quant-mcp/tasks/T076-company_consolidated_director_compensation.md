---
github_issue: N/A
title: 新增工具 get_company_consolidated_director_compensation（公司治理與內部人）
type: feature
priority: medium
status: done
depends_on:
...
# T076 - 新增工具 `get_company_consolidated_director_compensation`（公司治理與內部人）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_company_consolidated_director_compensation`，
提供「根據股票代號查詢上市公司合併報表董事酬金相關資訊。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
## 驗收標準
- [x] `tools/list` 可見 `get_company_consolidated_director_compensation`，inputSchema 與本任務附帶者語意一致
...
## 執行紀錄（2026-08-25 稽核）
- 驗收標準逐條對照程式碼與測試後勾選。
- 證據：registry 註冊＋TestAllToolsEnvelopeConsistent 全工具 probe、snapshots/raw/get_company_consolidated_director_compensation.json、TestAllToolsCacheConsistency 全工具覆蓋、go vet/go test 全綠。
