---
github_issue: N/A
title: 新增工具 get_company_income_statement（財務與基本面）
type: feature
priority: medium
status: done
depends_on:
...
# T092 - 新增工具 `get_company_income_statement`（財務與基本面）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_company_income_statement`，
提供「根據股票代號查詢上市公司綜合損益表。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
## 驗收標準
- [x] `tools/list` 可見 `get_company_income_statement`，inputSchema 與本任務附帶者語意一致
...
## 執行紀錄（2026-08-25 稽核）
- 驗收標準逐條對照程式碼與測試後勾選。
- 證據：registry 註冊＋TestAllToolsEnvelopeConsistent 全工具 probe、snapshots/raw/get_company_income_statement.json、TestAllToolsCacheConsistency 全工具覆蓋、go vet/go test 全綠。
