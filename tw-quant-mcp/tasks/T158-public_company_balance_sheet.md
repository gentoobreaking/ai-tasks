...
## 驗收標準
- [x] `tools/list` 可見 `get_public_company_balance_sheet`，inputSchema 與本任務附帶者語意一致
...
## 執行紀錄（2026-08-25 稽核）
- 驗收標準逐條對照程式碼與測試後勾選。
- 證據：registry 註冊＋TestAllToolsEnvelopeConsistent 全工具 probe、snapshots/raw/get_public_company_balance_sheet.json、TestAllToolsCacheConsistency 全工具覆蓋、go vet/go test 全綠。
