---
github_issue: N/A
title: 新增工具 get_company_energy_management（ESG 揭露細項）
type: feature
priority: low
status: done
depends_on: ["T037"]
...
# T082 - 新增工具 `get_company_energy_management`（ESG 揭露細項）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_company_energy_management`，
提供「根據股票代號查詢上市公司能源管理資訊。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
## 驗收標準
- [x] `tools/list` 可見 `get_company_energy_management`，inputSchema 與本任務附帶者語意一致
