---
github_issue: N/A
title: 新增工具 get_otc_daytrade_targets/stats（上櫃當沖標的與統計）
type: feature
priority: high
status: done
depends_on: []
...
# T201 - 新增工具 get_otc_daytrade_targets/stats（上櫃當沖標的與統計）

## 目標
查詢上櫃現股當沖標的名單與當日當沖交易統計。兩工具：get_otc_daytrade_targets（標的清單）、get_otc_daytrade_statistics（成交統計）；結果注入 scan_daytrade_eligibility 名單。
...
### 特別注意
注意：需同步修改 pkg/mcp/risk.go 之上櫃當沖名單來源。
