---
github_issue: N/A
title: 新增工具 get_broker_monthly_statements（券商資料）
type: feature
priority: low
status: done
depends_on:
...
# T052 - 新增工具 `get_broker_monthly_statements`（券商資料）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_broker_monthly_statements`，
提供「查詢各券商每月月計表。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
## 驗收標準
- [x] `tools/list` 可見 `get_broker_monthly_statements`，inputSchema 與本任務附帶者語意一致
