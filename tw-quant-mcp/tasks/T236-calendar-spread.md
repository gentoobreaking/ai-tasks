---
github_issue: N/A
title: 新增工具 get_calendar_spread_trades（期貨價差委託成交）
type: feature
priority: low
status: pending
depends_on: []
...
# T236 - 新增工具 get_calendar_spread_trades（期貨價差委託成交）

## 目標
查詢期貨價差委託每日概況與逐筆成交（kind 參數 summary/tick）。
...
| 端點 | 說明 |
|---|---|
| `DailyVolumeReportOnCalendarSpreadOrders` | 每日期貨價差委託成交概況表 |
| `TimeAndSalesDataOnCalendarSpreadOrders` | 每日期貨價差每筆成交資料 |
