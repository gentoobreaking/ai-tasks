...
# T156 - 新增工具 `get_otc_index`（上櫃市場）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_otc_index`，
提供「查詢櫃買市場（上櫃）指數歷史行情，包含開高低收、漲跌幅。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
## 驗收標準
- [x] `tools/list` 可見 `get_otc_index`，inputSchema 與本任務附帶者語意一致
...
## 執行紀錄（2026-08-25 稽核）
- 驗收標準逐條對照程式碼與測試後勾選。
- 證據：registry 註冊＋TestAllToolsEnvelopeConsistent 全工具 probe、snapshots/raw/get_otc_index.json、TestAllToolsCacheConsistency 全工具覆蓋、go vet/go test 全綠。
