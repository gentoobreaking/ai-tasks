---
github_issue: N/A
title: 新增工具 get_otc_history_ranks（上櫃歷史排行系列）
type: feature
priority: medium
status: done
depends_on: []
...
# T223 - 新增工具 get_otc_history_ranks（上櫃歷史排行系列）

## 目標
查詢上櫃歷史排行（rank 參數 pe/volume/amount/turnover/market_value/amount_avg/volume_avg；date 指定日期）。七排行共用模式可單工具多參數。
