---
github_issue: N/A
title: 新增工具 get_broker_electronic_trading_statistics（券商資料）
type: feature
priority: low
status: done
depends_on: ["T008"]
...
# T048 - 新增工具 `get_broker_electronic_trading_statistics`（券商資料）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_broker_electronic_trading_statistics`，
提供「查詢電子式交易統計資訊。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
## 驗收標準
- [x] `tools/list` 可見 `get_broker_electronic_trading_statistics`，inputSchema 與本任務附帶者語意一致
