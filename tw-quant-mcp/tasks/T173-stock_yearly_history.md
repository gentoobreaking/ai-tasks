---
github_issue: N/A
title: 新增工具 get_stock_yearly_history（行情歷史與指數）
type: feature
priority: medium
status: done
depends_on:
...
# T173 - 新增工具 `get_stock_yearly_history`（行情歷史與指數）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_stock_yearly_history`，
提供「查詢個股歷年成交資訊（最高價、最低價、收盤平均價），資料可回溯數十年。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
## 驗收標準
- [x] `tools/list` 可見 `get_stock_yearly_history`，inputSchema 與本任務附帶者語意一致
...
## 執行紀錄（2026-08-25 稽核）
- 驗收標準逐條對照程式碼與測試後勾選。
- 證據：registry 註冊＋TestAllToolsEnvelopeConsistent 全工具 probe、snapshots/raw/get_stock_yearly_history.json、TestAllToolsCacheConsistency 全工具覆蓋、go vet/go test 全綠。
