---
github_issue: N/A
title: 新增工具 get_market_historical_index（行情歷史與指數）
type: feature
priority: medium
status: done
depends_on:
...
# T143 - 新增工具 `get_market_historical_index`（行情歷史與指數）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_market_historical_index`，
提供「查詢發行量加權股價指數歷史資料。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
## 驗收標準
- [x] `tools/list` 可見 `get_market_historical_index`，inputSchema 與本任務附帶者語意一致
