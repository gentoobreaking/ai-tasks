---
github_issue: N/A
title: 新增工具 get_large_traders_futures_oi（期貨與選擇權）
type: feature
priority: medium
status: done
depends_on:
...
# T136 - 新增工具 `get_large_traders_futures_oi`（期貨與選擇權）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_large_traders_futures_oi`，
提供「查詢期貨大額交易人（前五大、前十大）未沖銷部位資料，可觀察大戶持倉方向。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
## 驗收標準
- [x] `tools/list` 可見 `get_large_traders_futures_oi`，inputSchema 與本任務附帶者語意一致
...
## 執行紀錄（2026-08-25 稽核）
- 驗收標準逐條對照程式碼與測試後勾選。
- 證據：registry 註冊＋TestAllToolsEnvelopeConsistent 全工具 probe、snapshots/raw/get_large_traders_futures_oi.json、TestAllToolsCacheConsistency 全工具覆蓋、go vet/go test 全綠。
