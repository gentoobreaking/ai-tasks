---
github_issue: N/A
title: 新增工具 get_time_and_sales（期貨/選擇權逐筆成交）
type: feature
priority: high
status: pending
depends_on: []
...
# T207 - 新增工具 get_time_and_sales（期貨/選擇權逐筆成交）

## 目標
查詢期貨/選擇權每筆成交資料（market 參數切換 futures/options；date 指定交易日）。tick 級資料，資料量大需 limit/offset 分頁與上限保護。
...
| 端點 | 說明 |
|---|---|
| `TimeAndSalesData` | 每日期貨每筆成交資料 |
| `OptionsTimeAndSalesData` | 每日選擇權每筆成交資料 |
...
### 特別注意
資料量大：單日可達數萬列，建議 limit 上限 1000 並於 description 提示指定 date。
