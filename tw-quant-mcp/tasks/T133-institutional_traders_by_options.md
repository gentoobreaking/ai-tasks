---
github_issue: N/A
title: 新增工具 get_institutional_traders_by_options（期貨與選擇權）
type: feature
priority: medium
status: done
depends_on:
...
# T133 - 新增工具 `get_institutional_traders_by_options`（期貨與選擇權）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_institutional_traders_by_options`，
提供「查詢三大法人依各選擇權契約分類的交易資料，可觀察各選擇權商品的法人買賣情況。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
## 驗收標準
- [x] `tools/list` 可見 `get_institutional_traders_by_options`，inputSchema 與本任務附帶者語意一致
...
## 執行紀錄（2026-08-25 稽核）
- 驗收標準逐條對照程式碼與測試後勾選。
- 證據：registry 註冊＋TestAllToolsEnvelopeConsistent 全工具 probe、snapshots/raw/get_institutional_traders_by_options.json、TestAllToolsCacheConsistency 全工具覆蓋、go vet/go test 全綠。
