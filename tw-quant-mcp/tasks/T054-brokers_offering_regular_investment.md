---
github_issue: N/A
title: 新增工具 get_brokers_offering_regular_investment（券商資料）
type: feature
priority: low
status: done
depends_on:
...
# T054 - 新增工具 `get_brokers_offering_regular_investment`（券商資料）

## 目標
對齊線上遠端 MCP（TWSEMCPServer）的同名工具，於本機 tw-quant-mcp 新增 `get_brokers_offering_regular_investment`，
提供「查詢開辦定期定額業務證券商名單。」之查詢能力。工具名稱**沿用遠端命名**以利對照。
...
- 登錄位置：`pkg/mcp/registry_broker.go（新建）`
...
## 驗收標準
- [x] `tools/list` 可見 `get_brokers_offering_regular_investment`，inputSchema 與本任務附帶者語意一致
