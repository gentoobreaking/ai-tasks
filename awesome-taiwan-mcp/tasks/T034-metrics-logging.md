...
- [ ] `internal/observability/` 套件建立
- [ ] Prometheus counter 指標實現:
...
- [ ] Prometheus histogram 指標實現:
  - `crawler_crawl_duration_seconds` (label: source)
  - `crawler_http_request_duration_seconds` (label: source, path, status_code)
- [ ] Prometheus gauge 指標實現:
  - `crawler_servers_total` (label: level)
  - `crawler_servers_healthy`
  - `crawler_servers_unhealthy`
- [ ] Structured JSON logger 實現, format:
  ```json
  {"level":"info","component":"github","crawl_id":"...","stage":"discover","event":"candidate_discovered","repository":"foo/bar"}
  ```
- [ ] Logger required fields: level, component, crawl_id, stage, event, timestamp
- [ ] Log redaction: API Key, OAuth token, password, Authorization header 必須被 mask (§43)
- [ ] 單元測試: 執行一次 crawl → at least 3 metrics > 0 (candidates_total, crawl_duration_seconds, http_requests_total) (§TST-060)
- [ ] 單元測試: source failure → log 包含 crawl_id, source, stage, error, timestamp (§TST-061)
- [ ] 單元測試: redaction 測試: log 中不包含 "Authorization" 或 token values
