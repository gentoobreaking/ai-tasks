...
- [ ] `tests/integration/` 目錄建立
- [ ] Integration test for GitHub adapter:
...
- [ ] Integration test for Official Registry adapter:
  - mock Registry API server → RawCandidate
- [ ] Integration test for SQLite storage:
...
- [ ] Integration test for MCP protocol handshake:
...
- [ ] Integration tests use httptest.Server or similar mock, NOT live Internet (§40 Task-040: Network tests 使用 mock server)
- [ ] Integration test: 失敗的 source → SOURCE_DEGRADED, crawl 繼續 (§TST-036)
