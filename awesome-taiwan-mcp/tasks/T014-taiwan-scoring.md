...
- [x] `internal/classify/rules.go` 中實現 `Score(server MCPServer) TaiwanRelevance` 函數
- [x] Scoring rules 實現 (§17):
...
- [x] Level mapping (§17):
  ...
  - score >= 5 → T1
  - score < 5 → T0
- [x] 每個 scoring rule 必須產生對應 evidence 記錄 (§16): type, source, location, matched_value, score, timestamp, content_hash
- [x] Confidence 設為 1.0 (deterministic scoring)
- [x] `thresholdToLevel(score float64) string` 輔助函數實現
- [x] 單元測試: 给定 fixture 與 expected score → actual_score == expected_score (§TST-017, 不是 range)
- [x] 單元測試: 100 iterations 執行相同 input → unique(scores)=1, unique(classification)=1 (§TST-018)
- [x] 單元測試: 100% scored rules have corresponding evidence (§TST-019)
- [x] 單元測試: T0 分類 → score < 5; T5 分類 → score >= 70
