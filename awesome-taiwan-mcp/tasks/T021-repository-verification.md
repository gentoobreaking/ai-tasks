...
- [ ] `internal/verify/repository.go` 建立
- [ ] `VerifyRepository(server *MCPServer) RepositoryVerificationResult` 函數實現
- [ ] Repository 驗證 checks (§25):
...
- [ ] RepositoryStatus 映射 (§27, §14 Registry Status):
  ...
  - 404/deleted → DELETED
  - otherwise → UNKNOWN
- [ ] Archived status 優先於時間判斷 (§TST-046: archived 優先)
- [ ] 每次驗證保存: check_type, result (pass/fail), HTTP status code 或錯誤訊息, timestamp
- [ ] Repository verification 結果保存到 SQLite health_checks / repositories 表
- [ ] Repository status 保存到 MCPServer.Repository.Status
- [ ] 單元測試: GitHub repo URL 200 → repository reachable = true
- [ ] 單元測試: GitHub repo 404 → status = DELETED (§TST-049)
- [ ] 單元測試: archived repo → status = ARCHIVED (§TST-046)
- [ ] 單元測試: last_commit < 90d → ACTIVE, 90–180d → MAINTENANCE, etc. (§TST-046: 100% mapping)
- [ ] 單元測試: 驗證過程中執行 commands = 0 (§TST-026: process execution count = 0)
