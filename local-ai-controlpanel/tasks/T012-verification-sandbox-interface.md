...
- [ ] `VerificationPlugin`（id / detect / run）與 `VerificationResult`（verifier / status / output / durationMs）實作
- [ ] 首批 verifier：GitDiff / UnitTest / Build / Lint / TypeVerifier 完成 detect+run
- [ ] `Sandbox` interface：`isAvailable()` + `run(context)`（§21.1 完整欄位：timeout / network / cpuLimit / memoryLimitMb / ability 等）
- [ ] `SandboxRegistry`（Factory pattern：register / get）＋ 預設註冊四種後端
- [ ] `selectSandbox`（§21.2 五步邏輯）實作
- [ ] verification 結果寫入 verification_results 表
- [ ] 驗證命令不得在 host 直接執行（rule-8 測試）
