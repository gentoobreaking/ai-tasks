...
- [ ] TopBar sandbox mode badge（來自 task 的 sandboxMode）
- [ ] `sandbox check` 指令顯示各後端可用狀態（配合 T016）
- [ ] approve dialog：BLOCK/ASK_USER/覆寫時跳出，送出後呼叫 approve API
- [ ] verification 事件 render 含 sandbox 名稱（§45.5）
- [ ] 指令面板補齊 `sandbox-check` 實作（T004 僅 UI 骨架）
