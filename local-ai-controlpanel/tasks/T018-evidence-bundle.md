...
- [x] Evidence / EvidenceBundle 型別依 §13 完整實作
- [x] Evidence Store（SQLite evidence 表，§27）寫入完整證據集
- [x] Shaping 規則（§12.2）：constraints/versions 完整保留；facts 依 relevance×confidence 由高到低保留至預算用盡；unresolvedQuestions 摘要單行
- [x] token 估算為 deterministic：`max(1, ceil(claim.length / 4))`（禁止 LLM 逐條估算，§13 註記）
- [x] 截斷時 `truncated = true` + `droppedFactIds` 記錄，且 unresolvedQuestions 追加「另有 N 筆證據因超過 token 預算未提供」
- [x] 截斷不影響 Evidence Store 完整保存（gate 用完整證據集，§12.2 註記）
- [x] evidence.max_tokens / min_relevance / budget_percent 由 policy 驅動（§30）
