...
## 驗收標準
- [x] 五張頁面齊備且資料來源為 sentinel 唯讀端點（整合測試驗證無直連 DB）
- [x] 綁定位址預設 127.0.0.1；設定改 0.0.0.0 時啟動印出醒目警告
- [x] 所有 handler 為 GET；滲透自查（無寫入端點）列入測試報告（spec.md §5 標準 6）
- [x] /accuracy 呈現每次 ETA 預測 vs 實際偏差統計（spec.md §5 標準 1c）
...
- [x] 五張頁面路由：`/`（全 SLO+容量+成本總表）、`/slo/{name}`（詳情+預測vs實際曲線）、`/accuracy`（命中統計）、`/cost`（燃盡圖+月/年推估表）、`/waste`（候選清單+結案統計）
- [x] 資料源：僅 sentinel 的唯讀 JSON 端點/status.json——整合測試斷言 UI process 從未開啟 SQLite 檔案（spec.md §2.5 安全鐵律）
- [x] bind 預設 127.0.0.1；設定為 0.0.0.0 時啟動印出醒目安全警告
- [x] 所有 handler 僅 GET；滲透自查清單（無寫入端點/無表單）列入測試報告（spec.md §5 標準 6）
- [x] uPlot 圖表資料走 JSON 端點；html/template 與 assets 以 go:embed 編譯進 binary
### 未竟項（下一批次）
- [x] /slo/{name} 詳情頁
