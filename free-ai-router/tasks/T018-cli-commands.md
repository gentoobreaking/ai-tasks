...
## 驗收標準
- [ ] `internal/cli/flags.go`: Parse all CLI flags (§10.2):
...
- [ ] `freemodel` (no args) — default interactive TUI
- [ ] `freemodel start [--port 7352]` — start router server (background mode)
- [ ] `internal/cli/best.go` — `--best` mode (§10.3):
...
- [ ] `internal/cli/status.go` — `freemodel status` (§12.3):
...
- [ ] `cli_test.go` with arg parsing for all CLI flags and subcommands
