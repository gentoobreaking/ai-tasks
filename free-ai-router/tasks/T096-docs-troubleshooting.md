...
## 驗收標準
- [x] 建立 `docs/TROUBLESHOOTING.md`（或納入 MANUAL.md 章節）
- [x] 涵蓋類別：
  - **模型顯示 down 但瀏覽器可用**：ping timeout 設定、TLS 驗證、企業代理、IPv6 問題
  - **API key 無效/過期**：各 provider key 格式、權限範權限範圍、輪換流程
...
- [x] 每個問題包含：現象、可能原因、診斷步驟、解決方案、預防措施
- [x] 提供 `freemodel doctor` 輸出解讀指南
- [x] 連結相關 GitHub Issue/PR 作為參考
