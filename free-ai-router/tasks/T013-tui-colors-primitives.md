...
## 驗收標準
- [ ] `internal/tui/colors.go`: ANSI color constants (green, yellow, red, orange, dim, etc.) and color helper functions
- [ ] Status dots per spec §6.5:
  - `*` green = up (HTTP 200)
  - `!` yellow = no auth (401)
  - `!` red = forbidden (403)
  ...
  - `x` red = down (5xx)
  - `.` dim = pending/no data
- [ ] Table cell primitive with truncation to column width (§6.4)
- [ ] Progress bar primitive for visual indicators
- [ ] Block primitive for bordered sections (settings screen, help overlay)
- [ ] Provider status tags: READY (green bg), NO KEY (yellow bg), WRONG KEY (red bg), OFF (dim bg) (§6.6)
