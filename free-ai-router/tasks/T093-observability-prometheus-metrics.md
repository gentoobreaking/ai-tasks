...
## 驗收標準
- [ ] `GET /api/metrics` 回傳 `text/plain; version=0.0.4; charset=utf-8` 格式
- [ ] 核心指標：
...
- [ ] 使用 `prometheus/client_golang` 標準庫（新增依賴）
- [ ] 指標更新頻率：ping 引擎每輪更新、router 每請求更新
- [ ] `/api/meta` 包含 `metricsEndpoint: "/api/metrics"`
- [ ] 單元測試驗證指標格式正確性
