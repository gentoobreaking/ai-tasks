...
## 驗收標準
- [x] 新增子命令：`freemodel config add-provider <provider> [--from-env] [--base-url <url>] [--model <id>]`
- [x] 內建模板涵蓋所有 `EnvOverrides` 列表的 provider（nvidia, groq, cerebras, openrouter, googleai, siliconflow, baidu, alibabacloud, tencent, kuaipao 等）
- [x] `--from-env`：自動讀取對應環境變數（如 `GROQ_API_KEY`）寫入 `apiKeys` 並啟用 provider
- [x] `--base-url`：支援自訂端點（如本地 vLLM、Ollama、自架 OpenAI 相容）
- [x] `--model`：預設模型 ID（用於 openai-compatible 類型）
- [x] 互動模式：不帶參數時顯示可用模板選單，引導填寫
- [x] 寫入前顯示將修改的配置差異，要求確認
