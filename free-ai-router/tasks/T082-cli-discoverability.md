...
## 驗收標準
- [x] `freemodel doctor`：健康檢查 — 檢查配置檔語法、API key 可用性、網路連線、ping 引擎狀態，輸出結構化報告
- [x] `freemodel providers`：列出所有 provider，顯示 key 狀態（env/config/none）、啟用狀態、模型數量、可發現性
- [x] `freemodel models`：非互動式列出所有模型，支援 `--tag coding`、`--provider groq`、`--tier S+` 過濾，輸出表格或 JSON
- [x] `freemodel best`：將現有 `--best` 邏輯遷移為子命令，保留原 flag 相容性
- [x] 所有新命令支援 `--json` 輸出格式供腳本使用
- [x] `freemodel --help` 顯示新命令並分組顯示（Core / Discovery / Config / Debug）
