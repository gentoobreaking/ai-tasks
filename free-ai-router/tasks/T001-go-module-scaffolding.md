...
## 驗收標準
- [ ] `go.mod` created with module path `github.com/freemodel/router` and Go 1.23
- [ ] `go.sum` with `golang.org/x/term v0.31.0` dependency
- [ ] `VERSION` file with initial version string (e.g., `v0.1.0`)
- [ ] `cmd/freemodel/main.go` with basic CLI dispatch (default TUI, `start`, `--best`, `--help`, `--version`)
- [ ] Directory structure matches spec: `internal/{config,providers,models,ping,router,tui,targets,cli}/` and `data/`
