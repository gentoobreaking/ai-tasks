...
## 驗收標準
- [ ] `data/sources.json`: All 12 providers (nvidia, groq, cerebras, opencode, openai-compatible, ollama, openrouter, codestral, scaleway, kilocode, kiro, googleai) with URL, discoverable flag, and static model lists
- [ ] `data/scores.json`: Offline quality scores (0-1) for ~150 models keyed by canonical model ID
- [ ] `data/model-tags.json`: Maps canonical model IDs to built-in tags from vocabulary (coding, reasoning, general, fast, agentic)
- [ ] `data/model-aliases.json`: Short alias → canonical model ID mapping (e.g., `"kimi-k2.5"` → `"moonshotai/kimi-k2.5"`)
