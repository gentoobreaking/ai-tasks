...
## 驗收標準
- [x] 替換 `internal/tui/render.go` 的 lipgloss 風格，使用色盲安全調色盤（推薦：Okabe-Ito 或 ColorBrewer Safe）
- [x] Status 欄位：🟢 up / 🔴 down / 🟡 pending / 🟠 timeout / ⚫ noauth — 同時顯示 emoji + 顏色
- [x] Tier 欄位：S+/S/A+/A/A-/B+/B/C 使用形狀差異（如 ◆/◇/■/□/●/○）輔助色彩
- [x] 搜尋高亮、選中行、標題列皆符合 WCAG AA 對比度（≥4.5:1）
- [x] 新增 `--no-color` / `--no-emoji` flag 供終端機不支援時降級
- [x] 單元測試驗證對比度（可用 `github.com/lucasb-eyer/go-colorful` 計算）
