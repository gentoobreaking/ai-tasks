...
## 驗收標準
- [x] 新增 `--log-format` flag：`text`（預設，人類可讀）| `json`（結構化）
- [x] JSON 格式每行一個物件，包含標準欄位：
  - `timestamp` (RFC3339), `level`, `message`
  - `model`, `provider`, `status`, `latency_ms`, `ttfb_ms`
  - `request_id`（每請求唯一，便於追蹤）
  - `usage`（token 統計，若有）
  - `error`（錯誤詳情，若有）
- [x] `--log` flag 同時啟用請求載體記錄（現有行為），JSON 格式下載體截斷 1024 字元
- [x] 環境變數 `FREMODEL_LOG_FORMAT` 覆蓋
- [x] 檔案輸出支援：`--log-file /var/log/freemodel.log`（可選，預設 stdout）
- [x] 日誌輪轉：可選整合 `lumberjack` 或由外部 logrotate 處理
