# `tw-quant-daybrain` 專案開發設計規格書 (v2.1.0)
...
| **v2.1** | 本版本為**外部 Code Review 的修正版**，未新增功能範疇，聚焦解決 v2.0 整併後遺留的內部不一致與實作缺口：① **Schema 去重**——`WatchlistTarget`（§14.1）收斂為輕量每日名單，移除與 `TacticalBriefing`（§9.1）重複且單位不一致的價位/風控欄位，改以 `briefing_ref` 指向後者作為唯一權威來源；§14 新增完整介面索引（§14.5），補齊 `TacticalBriefing`/`SignalCandidate`/`ExecutionDecision`/`MinuteBar`/`TradeRecord`/`ActivePosition` 六項先前未收錄的型別；② **回測模擬器補完空方邏輯**——§12.4 v2.0 版本訊號產生與出場判斷僅實作多方，`SHORT_ONLY` 日會靜默跑出 0 筆交易；v2.1 新增鏡射之空方進場條件、方向感知的停損/停利/損益計算，並以每筆持倉自帶的 `forceFlatBy` 取代寫死的 `13:10`；③ **修正一處會導致執行期錯誤的 MCP SDK 呼叫**——§5.4 `eligibility.content[0].json` 改為正確的 `JSON.parse(content[0].text)`；④ **統一貫穿文件的工具命名**——§5.2/§5.4 與 §2.2 契約表對齊（`get_stock_daily_kline`/`get_institutional_investors`/`get_taifex_night`+`get_us_market`）；⑤ **§8 評分表與 §6.3/§7.3 實作對齊**——移除從未實作的「PCR」「up-tick 比例」加分項，量能門檻由 3.0 統一為 2.5，並將全文件不一致的進場門檻（75 vs 80）統一為 75（§8.3、§11.2）；⑥ **`MAX_POSITIONS` 補上程式碼強制**——§10.3 `PriorityRankingEngine` 新增持倉檔數上限檢查，此限制過去僅存在於文件與環境變數；⑦ §6.3/§7.3 評分函式由 Python 改寫為 TypeScript，與 §17 技術選型一致；⑧ §9.2 `TacticalBriefing` interface 補上範例已有但型別漏宣告的 `volume_surge_threshold`。 |
| **實驗註記（T015）** | v2.1 之量能門檻 2.5 為 v2.0→v2.1 之 Code Review 決策；T015 建立參數實驗流程（`npm run experiment`，同一模擬日對 v2.0 基線 3.0 與候選 2.5/3.5 對比勝率/PF/假突破率），基線數據存於 `testdata/param_experiment_result.json`。**統計性結論待實際交易日資料**（模擬日 watchlist 僅 2 檔，樣本不足）；正式 v2.1 參數凍結需 T022 Grid Search + T023 WFO 驗證（§19 Phase 4）。 |
...
|---|---|---|---|
| **1. 日線趨勢** | `get_stock_daily_kline` | 股價位在 5MA 且 20MA 上方<br>股價位在 5MA 且 20MA 下方 | **+20**<br>**-20** |
...
// 統一的 MCP 回應解析：SDK 回傳的是 content[0].text（JSON 字串），
// 而非 content[0].json（v2.0 §5.4 曾誤用此寫法，已修正）。
// 正式實作應在此函式內先以 §3 Freshness Gate 驗證 _lineage 後才回傳 data。
function parseToolResult<T = any>(result: { content: Array<{ text: string }> }): T {
...
export async function evaluateDayTradeBias(mcpClient: Client, symbol: string): Promise<{ bias: DayTradeBias; score: number; rationale: string }> {
  // 1. 風控檢查
  const eligibilityRes = await mcpClient.callTool({
    name: "scan_daytrade_eligibility",
    arguments: { symbol }
  });
  const { can_daytrade, can_short_first, is_disposition } = parseToolResult<{
  ...
  // 2. 獲取日線籌碼與指標（工具名稱對齊 §2.2 契約：get_stock_daily_kline / get_institutional_investors）
  const techRes = await mcpClient.callTool({ name: "get_stock_daily_kline", arguments: { symbol, timeframe: "1D" } });
  const chipRes = await mcpClient.callTool({ name: "get_institutional_investors", arguments: { symbol, days: 3 } });
  const techData = parseToolResult<{ price: number; ma5: number; ma20: number }>(techRes);
  const chipData = parseToolResult<{ net_buy_sum: number }>(chipRes);
  ...
  // 3. 夜盤與 ADR 共振（§2.2 定義為兩個獨立 tool，分別呼叫後合併判讀，
  //    取代 v2.0 誤用的單一 get_overnight_market_status）
  const nightRes = await mcpClient.callTool({ name: "get_taifex_night", arguments: {} });
  const usRes = await mcpClient.callTool({ name: "get_us_market", arguments: {} });
  const nightData = parseToolResult<{ change_pct: number }>(nightRes);
  const usData = parseToolResult<{ tsm_adr_change_pct: number }>(usRes);

  if (nightData.change_pct > 0.5 && usData.tsm_adr_change_pct > 0) {
    biasScore += 25;
    logs.push("夜盤與美股ADR順風強勢 (+25)");
  } else if (nightData.change_pct < -0.5 && usData.tsm_adr_change_pct < 0) {
    biasScore -= 25;
    logs.push("夜盤與美股ADR逆風弱勢 (-25)");
  }

  // 4. 08:45 試撮驗證
  const preMarketRes = await mcpClient.callTool({ name: "get_pre_market_quote", arguments: { symbol } });
  const preMarket = parseToolResult<{ estimated_change_pct: number }>(preMarketRes);
...
> 與 §8.2 為同一套評分表的多方欄位（4×25 分制），此處為其 TypeScript 實作；不是另一套獨立邏輯。

```typescript
interface LongSignalData {
  price: number;
  vwap: number;
  volumeSurgeRatio: number;
  dayHigh: number;
  taifexTrend: "BULLISH" | "BEARISH" | "NEUTRAL";
  distanceToLimitUpPct: number; // 距漲停剩餘幅度，如 0.015 = 1.5%
}

function evaluateLongSignal(data: LongSignalData): { score: number; shouldEnter: boolean } {
  let score = 0;
  if (data.price > data.vwap) score += 25;
  if (data.volumeSurgeRatio >= 2.5) score += 25;
  if (data.price >= data.dayHigh) score += 25;
  if (data.taifexTrend === "BULLISH") score += 25;

  // 風控扣分項：距離漲停不到 1.5%，利潤空間不足，大幅扣分（Veto）
  if (data.distanceToLimitUpPct < 0.015) score -= 50;

  return { score, shouldEnter: score >= 75 }; // 超過 75 分發送「建議進場」訊號
}
```
...
> 與 §8.2 為同一套評分表的空方欄位（4×25 分制），此處為其 TypeScript 實作；不是另一套獨立邏輯。

```typescript
interface ShortSignalData {
  price: number;
  vwap: number;
  volumeSurgeType: "BEARISH_BREAKDOWN" | "BULLISH_SURGE" | "NONE";
  dayLow15m: number; // 開盤前 15 分鐘之盤中最低點
  taifexTrend: "BULLISH" | "BEARISH" | "NEUTRAL";
  priceChangePct: number; // 今日漲跌幅，如 6.5 = +6.5%
}

function evaluateShortSignal(data: ShortSignalData): { score: number; shouldEnter: boolean } {
  let score = 0;
  if (data.price < data.vwap) score += 25;
  if (data.volumeSurgeType === "BEARISH_BREAKDOWN") score += 25;
  if (data.price < data.dayLow15m) score += 25;
  if (data.taifexTrend === "BEARISH") score += 25;

  // 空方致命風控扣分（防止被軋空）：今日已大漲 >= 6.5%、接近漲停時嚴禁放空（Veto）
  if (data.priceChangePct >= 6.5) score -= 100;

  return { score, shouldEnter: score >= 75 }; // 超過 75 分發送「建議做空」訊號
}
```
...
- **完整評分僅在雙 tick 確認後執行**。
- **與 §6.3/§7.3 為同一張表**：下表即 `evaluateLongSignal` / `evaluateShortSignal` 的完整條件與分數，§6.3/§7.3 是各自方向的 TypeScript 實作；三處數字須保持一致，改動任一處須同步更新其餘兩處（v2.0 版本三處數字曾不一致，已於 v2.1 校正）。
- **與 §10 的區別**：本節評分回答「這個標的的訊號夠不夠格進場」（單一標的、veto-based，門檻見 §8.3）；§10 Priority Ranking Engine 回答「多個都夠格的訊號裡，誰優先拿到資金」（跨標的資源分配）。兩者評分尺度與用途皆不同，不需要也不應該共用同一組分數。

### 8.2 評分表（預設值）

| 項目 | 做多條件（§6.3 `evaluateLongSignal`） | 做空條件（§7.3 `evaluateShortSignal`） | 分數 |
|---|---|---|---|
| 位階 | 價 > VWAP | 價 < VWAP | +25 |
| 量能 | `volumeSurgeRatio ≥ 2.5`（mcp `detect_volume_surge`） | `volumeSurgeType == BEARISH_BREAKDOWN` | +25 |
| 突破/破位 | 價 ≥ 當日高點 | 價 < 開盤前 15 分鐘低點 | +25 |
| 大盤方向 | 台指期當前 1 分 K 為紅棒（`taifexTrend == BULLISH`） | 台指期當前 1 分 K 為黑棒（`taifexTrend == BEARISH`） | +25 |
| 風控 Veto（多方限定） | 距漲停 < 1.5%（利潤空間不足） | — | -50（扣分，非否決） |
| 風控 Veto（空方限定） | — | 今日漲幅 ≥ 6.5%（防軋空鎖死） | -100（否決） |
| 風控 Veto（通用） | 處置/注意/當沖限制/停資停券 | 處置/注意/當沖限制/停資停券 | -100（否決） |

> v2.0 版本本表另列「大盤順風：PCR > 100%」與「買盤結構：up-tick 比例 ≥ 70%」兩項加分條件，但 §6.3/§7.3 程式碼從未實作對應計算，屬於文件與程式碼不同步。v2.1 移除該兩項，改列為 Roadmap 候選增強項（見 §19）；若日後要納入，須先在 §6.3/§7.3 補上對應欄位與計算邏輯，再回填本表。

### 8.3 門檻與行為

| 分數 | 等級 | 行為 |
|---|---|---|
| ≥ 75 | `STRONG_BUY` / `STRONG_SELL` | 產出進場建議（含觸發價、目標價 R:R ≥ 2:1、停損價） |
| 60 – 74 | `WATCH` | 僅記錄，不建議進場 |
| < 60 | `IGNORE` | 忽略 |

- 75 分即 §6.3/§7.3 程式碼中 `shouldEnter` 的判定門檻；`NEUTRAL_FLEXIBLE` 日進場門檻提高至 85 分（§5.3）。
...
export interface TacticalBriefing {
  ...
  trading_plan: {
    ...
    key_levels: {
      ...
      volume_surge_threshold: number; // v2.1 補上（v2.0 範例 JSON 已有此欄位，interface 漏宣告，§13 Grid Search 依賴此欄位）
    };
  };
  risk_guardrails: {
...
export class TacticalBriefingGenerator {
  ...
  public async generate(symbol: string): Promise<TacticalBriefing> {
    ...
    const breakdown = [
      ...
      { factor: "OVERNIGHT_MARKET", score: 0, detail: "台指夜盤與 ADR 無明顯共振" },
...
export class PriorityRankingEngine {
  ...
  constructor(totalMarginPoolNtd: number = 3000000, maxLeverage: number = 2.0, maxPositions: number = 2) {
    this.totalMarginPoolNtd = totalMarginPoolNtd;
    this.maxPortfolioExposureNtd = totalMarginPoolNtd * maxLeverage;
    this.maxPositions = maxPositions; // 對齊 §11.1 / 環境變數 MAX_POSITIONS（v2.0 僅寫在文件與 env，程式碼未檢查，v2.1 補上）
  }
  ...
  public evaluateSignal(
    ...
    // 2. 持倉檔數上限（MAX_POSITIONS，v2.1 新增）
    if (this.currentActivePositions.size >= this.maxPositions && !this.currentActivePositions.has(candidate.symbol)) {
      return { shouldExecute: false, symbol: candidate.symbol, rankScore: 0, allocatedCapitalNtd: 0, reason: `已達最大同時持倉檔數上限 (MAX_POSITIONS=${this.maxPositions})` };
    }
...
```text
IDLE → SCANNING → ARMED(觸發價設好) → TRIGGERED(價≥觸發價且評分≥75，門檻定義見 §8.3)
...
export interface ActivePosition {
  ...
  rankScore: number;
  forceFlatBy: string; // v2.1 新增：取自對應 briefing.trading_plan.active_window.force_flat_by（多空不同，見 §7.4/§11.5）
}
...
export class DayBrainBacktestSimulator {
  ...
  public runSimulation(marketData: Map<string, MinuteBar[]>) {
    ...
    const runningStats = new Map<string, { vwapVolumeSum: number; vwapValueSum: number; dayHigh: number; dayLow: number; first15mLow: number; volumes: number[] }>();
    ...
    for (const timeStr of timestamps) {
      ...
      for (const [symbol, bars] of marketData.entries()) {
        ...
        if (timeOnly <= "09:15") {
          stats.first15mLow = Math.min(stats.first15mLow, currentBar.low); // §7.2 條件3之基準：開盤前15分鐘低點，之後凍結
        }
        ...
        // 觸發多方條件：突破 VWAP + 爆量 >= threshold + 逼近當日高點（僅在 Briefing 允許 BUY_TO_OPEN 時檢查）
        if (
          briefing.trading_plan.allowed_actions.includes("BUY_TO_OPEN") &&
          currentBar.close > currentVwap && surgeRatio >= surgeThreshold && currentBar.close >= stats.dayHigh * 0.998
        ) {
        ...
        // 觸發空方條件：跌破 VWAP + 爆量 >= threshold + 跌破開盤前15分鐘低點（v2.1 新增，鏡射多方；僅在 Briefing 允許 SELL_TO_OPEN 時檢查）
        else if (
          briefing.trading_plan.allowed_actions.includes("SELL_TO_OPEN") &&
          currentBar.close < currentVwap && surgeRatio >= surgeThreshold && currentBar.close <= stats.first15mLow * 1.002
        ) {
          candidatesThisMinute.push({
            symbol,
            action: "SELL_TO_OPEN",
  ...
  private checkExitConditions(pos: ActivePosition, bar: MinuteBar, timeOnly: string) {
    ...
    if (isLong) {
      ...
      // 空單：停損在上方（價漲觸發），停利在下方（價跌觸發）——鏡射多方，見 §7.3
      if (bar.high >= pos.stopLossPrice) {
        shouldExit = true; exitReason = "STOP_LOSS"; exitPrice = pos.stopLossPrice;
      } else if (bar.low <= pos.targetPrice1) {
        shouldExit = true; exitReason = "TAKE_PROFIT"; exitPrice = pos.targetPrice1;
      } else if (timeOnly >= pos.forceFlatBy) {
        // 空單 forceFlatBy 應早於多方（§7.4：13:00 而非 13:10），確保收盤集合競價前完成回補
        shouldExit = true; exitReason = "FORCE_FLAT"; exitPrice = bar.close;
      }
    }

    if (shouldExit) {
      // 空單獲利方向相反：進場價 - 出場價；多單則為 出場價 - 進場價
...
> **v2.1 變更**：v2.0 版本 `WatchlistTarget` 曾同時記錄 `trigger_price`／`stop_loss_price`／`take_profit_price`／`position_size_shares`，與 §9.1 `TacticalBriefing` 的對應欄位（單位、精度皆不同——前者是絕對價格，後者部分為百分比）重複記錄，且文件未定義何者為準；§4 Phase 1 同時產出兩者，也未說明關係。v2.1 將 `WatchlistTarget` 收斂為**輕量每日名單**（回答「今天在追蹤哪些標的、方向、風險狀態」），移除重複的價位/風控欄位，改以 `briefing_ref` 單向指向該標的當日的 `TacticalBriefing`——**價位與風控參數的唯一權威來源是 `TacticalBriefing`**，`WatchlistTarget` 不再重複記錄。
  ...
  "watchlist": [
    {
      ...
      "risk_status": "CLEAR",
      "briefing_ref": "briefings/2026-08-03_2308.json"
    }
  ]
}
```
...
```json
{
  ...
  "grade": "STRONG_BUY",
  "score": 85,
  "score_breakdown": { "level": 25, "volume": 25, "breakout": 25, "market": 25, "veto_penalty": 5 },
  "strategy": "VWAP_SURGE_LONG",
...
> `score_breakdown` 之四項（`level`/`volume`/`breakout`/`market`）對齊 §8.2 評分表與 §6.3/§7.3 程式碼之 4×25 分制；`veto_penalty` 為風控扣分項（若觸發）。v2.0 版本曾寫 `tick_structure`，對應 §8.2 已移除的 up-tick 條件，v2.1 一併修正。
...
> v2.0 版本本節僅收錄下表前 4 項；`TacticalBriefing`、`SignalCandidate`、`ExecutionDecision`、`MinuteBar`、`TradeRecord`、`ActivePosition` 雖被 §9/§10/§12 大量使用，卻從未列在「介面設計」章節中。v2.1 補上完整索引——僅列出處與一句話用途，型別本體維持定義在各自最相關的章節，避免同一型別出現兩份定義需要互相同步。

| 介面 | 用途 | 定義於 |
|---|---|---|
| `WatchlistTarget` | 當日輕量觀察名單（方向、風險狀態、指向 Briefing） | §14.1 |
| `TacticalBriefing` | 當日單一標的完整戰術物件（權威價位與風控參數） | §9.1 / §9.2 |
| `SignalAdvice` | 盤中單次進場建議（含評分明細） | §14.2 |
| `SignalCandidate` | 盤中候選訊號（尚未經 Priority Ranking 核准） | §10.3 |
| `ExecutionDecision` | Priority Ranking Engine 之核准/拒絕結果 | §10.3 |
| `Position` | 已建立之持倉（狀態機當前狀態） | §14.3 |
| `ActivePosition` | 回測模擬器內部持倉狀態（含最高/最低價追蹤） | §12.4 |
| `JournalEntry` | 結構化交易日誌（LLM 報告之唯一統計來源） | §14.4 |
| `MinuteBar` | 歷史 1 分 K 線（回測輸入） | §12.2 |
| `TradeRecord` | 回測完成交易紀錄 | §12.2 |
