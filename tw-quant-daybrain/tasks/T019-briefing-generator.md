...
- [ ] 08:55 自動產出 `briefings/YYYY-MM-DD_SYMBOL.json`（§9.1 範例格式，每標的乙份）
- [ ] Bias 對應規則（§9.2）：NO_TRADE → allowed 空 / LONG_ONLY → 僅 BUY_TO_OPEN / SHORT_ONLY → 僅 SELL_TO_OPEN / NEUTRAL_FLEXIBLE → 雙向
- [ ] 動態時間窗（§9.2）：`force_flat_by` 依 bias 為 `SHORT_ONLY ? "13:00" : "13:10"`
- [ ] 防呆機制（§9.3）：盤中找不到當日 briefing → Agent 拒絕啟動交易；Action 白名單第一關攔截；風控參數動態載入（不硬編碼）
- [ ] 產出寫入 `briefing_generated` 事件（T004）
- [ ] 單元測試：四種 bias 之 allowed/blocked 對應、JSON Schema 驗證、缺檔拒絕啟動邏輯
