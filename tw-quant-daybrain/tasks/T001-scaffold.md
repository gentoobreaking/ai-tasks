...
## 目標
建立 `tw-quant-daybrain` TypeScript 專案骨架（§10），含設定載入（yaml + 環境變數覆寫）、目錄結構、日誌基礎，可啟動但不連 MCP 的最小進程。

## 驗收標準
- [ ] Node.js ≥ 20 + TypeScript 專案初始化於 `~/Projects/tw-quant-daybrain`；依賴含 `@modelcontextprotocol/sdk`
- [ ] 目錄結構：`src/{mcp,gate,engine,risk,metrics,llm,scheduler,logging}/`、`config/*.yaml`、`logs/`
- [ ] 設定載入：`config/scoring.yaml`（§5 評分表）、`config/scheduler.yaml`（§11.2 排程）、環境變數覆寫（§10.1 全部變數，含預設值）
