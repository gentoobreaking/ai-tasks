...
## 驗收標準
- [x] DuckDB 資料庫檔案可透過 `DUCKDB_PATH` 環境變數指定
- [x] 8 張表格全部建立，含正確的 PRIMARY KEY、資料型別與預設值
- [x] 提供 `init_db()` 函式，支援「已存在則跳過」（idempotent）
- [x] Pydantic models 對應各 table row 結構，含欄位驗證
- [x] 提供 `Database` class 封裝 DuckDB 連線，支援 context manager
